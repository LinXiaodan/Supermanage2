        ...
        for key in reduce_set:
            ...
            return_list.update({  # 商品金额
                key: {'goods_id': goods_id,
                      'goods_name': stock_object.goods_name,
                      'goods_quantity': goods_quantity,
                      'buying_price': stock_object.buying_price,
                      'tot_price': round(stock_object.buying_price * goods_quantity, 2)}
            })
...
def buy_list_db_query(buy_id):
    ...
    """
    try:
        res = Buying.objects.filter(buy_id=buy_id)
        if res:
            return_set = {
                'status': 'success'
            }
    ...
    try:
        ...
        if res:
            ...
            return_set.update({
                'list': return_list,
                'total_num': num,
                'total_price': num * stock_obj.buying_price,
                'goods_type': stock_obj.goods_type,
                'goods_name': stock_obj.goods_name,
                'unit': stock_obj.unit,
                'quantity': stock_obj.quantity,
                'buying_price': stock_obj.buying_price,
                'price': stock_obj.price,
                'goods_id': goods_id
            })
            return return_set
...
# 退货记录查询
def return_db_query(goods_id):
    """
            获取：商品编号
            返回： {
                'status': 'success'/'fail',
                'list': {0: {return_id, time, goods_quantity, return_user}
                         1:......},
                'total_num': 退货总数量,
                'total_price': 退货总额,
                'goods_type': 商品种类,
        ...
        """
    try:
        res = ReturnGoods.objects.filter(goods_id=goods_id)
        ...
        if res:
            ...
            for var in res:
                return_list.update({
                    count: {
                        'return_id': var.return_id,
                        'time': var.time,
                        'goods_quantity': var.goods_quantity,
                        'return_user': var.username
                    }
                })
                count += 1
                num += var.goods_quantity
